...
} from "../src/services/collection.services";
...
import {
  createCollection,
  getSellerItems,
} from "../src/controllers/collection.controller";
import {
  createProduct,
  getAvailableItems,
  searchProducts,
...
describe("GetCollectionService", () => {
  ...
  describe("getAllCollections", () => {
    it("should return all collections", async () => {
      const findAllStub = sinon
        .stub(Collection, "findAll")
        .resolves(mockCollections as any);
  ...
  describe("getSellerItems", () => {
    it("should return all items for a seller", async () => {
      ...
      expect(findAllStub.calledOnceWith({ where: { seller_id: "123" } })).toBe(
        true
      );
      expect(result).toEqual(mockCollections);
  ...
  describe("getAvailableItems", () => {
    ...
    it("should return all available items", async () => {
      const findAllStub = sinon
        ...
        .resolves(mockItems as any);

      const result = await ProductService.getAvailableItems();

      expect(
        findAllStub.calledOnceWith({ where: { availability: true } })
      ).toBeTruthy();
  ...
  it("should search products by name", async () => {
    ...
    expect(
      getProductsStub.calledOnceWith({
        where: { productName: { [Op.iLike]: "%Test Product%" } },
      })
    ).toBe(true);
...
describe("getAvailableProducts", () => {
  ...
  it("should return 404 if no available products are found", async () => {
    ...
    expect(
      jsonStub.calledOnceWith({
        message: "No available products found for this seller.",
      })
    ).toBe(true);
  });
});
...
describe("updateProductAvailability", () => {
  ...
  it("should return 404 if product is not found", async () => {
    ...
    expect(statusStub.calledOnceWith(404)).toBe(true);
    expect(
      jsonStub.calledOnceWith({ status: "fail", message: "Product not found" })
    ).toBe(true);
  });
...
describe("deleteProduct", () => {
  ...
  it("should delete the product and return 202 if the user is the owner", async () => {
    ...
    sandbox
      ...
      .resolves(deletedProduct as any);
...
describe("updateProduct", () => {
  ...
  it("should return 200 and update the product successfully", async () => {
    productServiceStub.resolves({
      seller_id: "seller123",
      update: productStub.update,
    });
...
describe("addImages function", () => {
  ...
  beforeEach(() => {
    ...
    getProductByIdStub = sinon
      .stub(ProductService, "getProductByid")
      .resolves(productMock);
    uploadImagesStub = sinon
      .stub(imageServices, "uploadImages")
      .resolves(["url3", "url4"]);
  });
  ...
  it("should add images successfully", async () => {
    ...
    sinon.assert.calledWith(productMock.update, {
...
describe("updateImageByUrl", () => {
  ...
  it("should return 400 if the image does not exist in product", async () => {
    ...
    expect(jsonStub.calledWith({ message: "The image doesn't exist" })).toBe(
      true
    );
  });
  ...
  it("should return 400 if no files are uploaded", async () => {
    ...
    expect(statusStub.calledWith(400)).toBe(true);
    expect(jsonStub.calledWith({ message: "Please upload new image" })).toBe(
      true
    );
  });
  ...
  it("should return 500 if there is an internal server error", async () => {
    ...
    expect(jsonStub.calledWith({ message: "internal server error" })).toBe(
      true
    );
  });
});
...
describe("UpdateAllImages function", () => {
  ...
  beforeEach(() => {
    ...
    uploadImagesStub = sinon
      ...
      .resolves(["url3", "url4"]);
    destroyStub = sinon
      .stub(cloudinary.v2.uploader, "destroy")
      .resolves({ result: "ok" });
  });
...
describe("removeImage function", () => {
  ...
  beforeEach(() => {
    ...
    getProductByIdStub = sinon
      .stub(ProductService, "getProductByid")
      .resolves(productMock);
    obtainPublicIdStub = sinon
      .stub(imageServices, "obtainPublicId")
      .returns("sample");
    destroyStub = sinon
      .stub(cloudinary.v2.uploader, "destroy")
      .resolves({ result: "ok" });
  ...
  it("should return 400 if the image does not exist in product images", async () => {
    productMock.images = [
      "http://res.cloudinary.com/demo/image/upload/another.jpg",
    ];
...
describe("viewProductBySeller", () => {
  ...
  it("should return 400 if product does not belong to collection", async () => {
    ...
    sinon.assert.calledOnceWithExactly(statusStub, 400);
    sinon.assert.calledOnceWithExactly(jsonStub, {
      message: "The product doesn't exist in your collection",
    });
  });
...
describe("reviewProduct", () => {
  ...
  it("should update the review if an existing review is found", async () => {
    const product = {
      reviews: [{ user: "123", review: "Old review", rating: 3 }],
      save: saveStub,
    };
    getProductByIdStub.resolves(product as any);
  ...
  it("should return 404 if product is not found", async () => {
    ...
    expect(statusStub.calledOnceWith(404)).toBe(true);
    expect(jsonStub.calledOnceWith({ message: "Product not found" })).toBe(
      true
    );
  });
  ...
  it("should return 400 if review or rating is not provided", async () => {
    ...
    expect(statusStub.calledOnceWith(400)).toBe(true);
    expect(
      jsonStub.calledOnceWith({ message: "Please provide a review and rating" })
    ).toBe(true);
  });
  ...
  it("should return 400 if rating is not between 1 and 5", async () => {
    ...
    expect(statusStub.calledOnceWith(400)).toBe(true);
    expect(
      jsonStub.calledOnceWith({ message: "Rating should be between 1 and 5" })
    ).toBe(true);
  });
});
...
describe("getReviews", () => {
  ...
  it("should return 200 and the reviews if found", async () => {
    const product = {
      reviews: [{ review: "Great product", rating: 5, user: "123" }],
    };
    getProductByIdStub.resolves(product as any);
...
describe("deleteReview", () => {
  ...
  it("should delete the review if found", async () => {
    const product = {
      reviews: [{ id: "0", review: "Great product", rating: 5, user: "123" }],
      save: saveStub,
    };
    getProductByIdStub.resolves(product as any);
  ...
  it("should return 404 if review is not found", async () => {
    const product = {
      reviews: [{ id: "1", review: "Another review", rating: 4, user: "124" }],
      save: saveStub,
    };
    getProductByIdStub.resolves(product as any);
